...
def add_indent(commands, indent):
    ...
    for command in commands:
        for line in command.split('\n'):
            indented_line.append('{0}{1}'.format(indent, line))
...
def print_python(commands):
    ...
    if os.environ.get('REMOTE_EXECUTION_SHOW_PYTHON'):
        ...
        sys.stdout.write(
            '{0}(kwargs={1})\n'.format(function_name,json.dumps(kwargs, indent=2, default=lambda element: type(element).__name__)))

        # write out the code with the lines numbers
        for index, line in enumerate(commands, 1):
            sys.stdout.write('{0} {1}\n'.format(index,line))

        sys.stdout.write('{0}{1}{0}\n'.format(dashes,"-" * len(label)))
    ...
    if not os.environ.get('TEST_ENVIRONMENT'):
        if not is_connected():
            ...
            result = run_commands(
                [
                    'import sys',
                    'sys.path.append(r"{0}")'.format(dependencies_path),
...
class Unreal:
    ...
    def get_asset(asset_path):
        ...
        if not asset:
            raise RuntimeError("The {0} does not exist in the project!".format(asset_path))
        return asset
...
class UnrealImportAsset(Unreal):
    ...
    def set_physics_asset(self):
        ...
        default_physics_asset = '{0}_PhysicsAsset'.format(asset_path)
...
class UnrealRemoteCalls:
    ...
    def import_skeletal_mesh_lod(asset_path, file_path, index):
        ...
        if result == -1:
            raise RuntimeError("{0} import failed!".format(file_path))
    ...
    def get_lod_build_settings(asset_path, index):
        ...
        mesh = Unreal.get_asset(asset_path)
        if not mesh:
            raise RuntimeError('"{0}" was not found in the unreal project!'.format(asset_path))
        if mesh.__class__.__name__ == 'SkeletalMesh':
