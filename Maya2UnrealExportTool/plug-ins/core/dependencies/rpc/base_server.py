...
def run_in_main_thread(callable_instance, *args):
    ...
    if RETURN_VALUE_NAME not in globals():
        raise TimeoutError(
            'The call "{0}" timed out because it hit the timeout limit'
            ' of {1} seconds.'.format(callable_instance.__name__, timeout)
        )
...
class BaseRPCServer:
    def __init__(self, name, port, is_thread=False):
        ...
        logger.info('Started RPC server "{0}".'.format(name))
    ...
    def add_new_callable(self, callable_name, code, client_system_path, remap_pairs=None):
        ...
        return 'The function "{0}" has been successfully registered with the server!'.format(callable_name)
...
class BaseRPCServerManager:
    ...
    def start(self, threaded=True):
        ...
        # start the blocking server
            ...
            logger.info('RPC server "{0}" is already running...'.format(self.name))
    ...
    def shutdown(self):
        ...
        if self.server_thread:
            logger.info('RPC server "{0}" is shutting down...'.format(self.name))
            ...
            logger.info('RPC server "{0}" has shutdown.'.format(self.name))
